@echo off
chcp 65001 >NUL
REM JinDX Windows 后台版启动脚本（免 Python，免 pip）
REM
REM 用法:
REM   start-backend.bat
REM   set DEEPSEEK_KEY=sk-xxx && start-backend.bat
REM   set PROXY_PORT=9000 && start-backend.bat
REM
REM 功能:
REM   1. 自动检查/配置 DeepSeek API Key（无 key 时交互式输入）
REM   2. 启动 JinDX 代理服务（proxy-backend.exe，已内嵌全部依赖）
REM   3. 输出 Codex CLI 环境变量配置
REM
REM 注意：hosts 劫持和端口转发需要管理员权限，请先以管理员身份运行：
REM   .\start-backend.ps1  （推荐，带 hosts 劫持和端口转发）
...
if "%NEED_KEY%"=="1" (
    ...
    if "%DEEPSEEK_KEY%"=="" (
        ...
        echo      可通过环境变量设置: set DEEPSEEK_KEY=sk-xxx ^&^& start-backend.bat
        pause
        exit /b 1
    )
    REM 持久化到配置文件
...
echo.
echo ==========================================
echo   JinDX Proxy for Windows ^(后备版^)
echo ==========================================
echo.
...
echo     codex
echo.
echo   推荐以管理员身份运行 start-backend.ps1（自动配置 hosts 劫持 + 端口转发）
echo.
